...
branch="compaction_blocks_to_lru_end"
block_sizes=( 4096 16384 )
for block_size in "${block_sizes[@]}"; do 
  # drop pagecache and dentries 
  sync; echo 3 | sudo tee /proc/sys/vm/drop_caches
  ../build/db_bench  --block_size=16384 --compression_type=none -mix_get_ratio=0.1 -mix_seek_ratio=0.1 -mix_put_ratio=0.8  --benchmarks=mixgraph -num=2000000 -readwritepercent=50 -cache_size=104857600
  ...
  #   # get total cache hit  rate 
    awk 'BEGIN{total =0 ; hit_count=0;}{total+=1 ; if($14 == 1){hit_count +=1;}} END{printf("%.6f\n", hit_count/total)}' $caller_log > /tmp/compact_block_trace/block_cache_total_hit_rate_${branch}_${block_size}_${i}
