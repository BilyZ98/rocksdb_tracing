...
DEFINE_string(evict_block_cache_trace_path, "", "Evict block cache trace path");
...
DEFINE_string(human_readable_trace_file_path, "",
              "The file path that saves human readable access records.");

DEFINE_string(evict_human_readable_trace_file_path, "", "The file path for evict_"
    "block_cache_trace.");
...
BlockCacheTraceAnalyzer::BlockCacheTraceAnalyzer(
    ...
    const std::string& evict_trace_path,
    ...
    : env_(ROCKSDB_NAMESPACE::Env::Default()),
      trace_file_path_(trace_file_path),
      evict_trace_file_path_(evict_trace_path),
      output_dir_(output_dir),
      human_readable_trace_file_path_(human_readable_trace_file_path),
      compute_reuse_distance_(compute_reuse_distance),
      mrc_only_(mrc_only),
...
Status BlockCacheTraceAnalyzer::RecordEvictAccess(
  ...
  return evict_human_readable_trace_writer_.WriteHumanReadableTraceRecord(access, 
      block_access_info.block_id, 
      0);
  // Status s;
  // return s;
...
void BlockCacheTraceAnalyzer::SetEvictTraceFile(std::string &evict_file_path) {
  evict_trace_file_path_ = evict_file_path;
}

void BlockCacheTraceAnalyzer::SetEvictTraceOutputHumanFile(std::string &evict_human_file) {
  evict_human_readable_trace_file_path_ = evict_human_file;
  fprintf(stdout, "evict human file %s\n", evict_human_readable_trace_file_path_.c_str());
}

Status BlockCacheTraceAnalyzer::AnalyzeWithEvict() {
  SystemClock* clock = env_->GetSystemClock().get();
  std::unique_ptr<BlockCacheTraceReader> reader;
  std::unique_ptr<BlockCacheTraceReader> evict_reader;
  Status s = Status::OK();
  if (is_human_readable_trace_file_) {
    ...
    std::unique_ptr<TraceReader> evict_trace_reader;
    s = NewFileTraceReader(env_, EnvOptions(), evict_trace_file_path_, &evict_trace_reader);
    reader.reset(new BlockCacheTraceReader(std::move(trace_reader)));
    evict_reader.reset(new BlockCacheTraceReader(std::move(evict_trace_reader)));
    s = reader->ReadHeader(&header_);
    if (!s.ok()) {
      return s;
    }
    s = evict_reader->ReadHeader(&evict_header_);
    if(!s.ok()) {
      return s;
    }
  }
  ...
  assert(!evict_human_readable_trace_file_path_.empty());
  s = evict_human_readable_trace_writer_.NewWritableFile(evict_human_readable_trace_file_path_, env_); 
  if(!s.ok()) {
    return s;
  }
  ...
  if(!s.ok()) {
    printf("status not ok %s\n",s.ToString().c_str());
  }
  s = Status::OK();
  while(s.ok()) {
    BlockCacheTraceRecord access;
    s = evict_reader->ReadAccess(&access);
    if(!s.ok()) {
      fprintf(stdout, "read evcit access failed: %s\n", s.ToString().c_str());
      break;
    }
    if(!mrc_only_) {
      s = RecordEvictAccess(access);
      if(!s.ok()) {
        fprintf(stdout, "read evcit access failed: %s\n", s.ToString().c_str());
        break;
      }
  ...
  now = clock->NowMicros();
  duration = (now -start) / kMicrosInSecond;
  fprintf(stdout, "Running evict trace for %" PRIu64 " seconds\n", duration);
...
int block_cache_trace_analyzer_tool(int argc, char** argv) {
  ...
  analyzer.SetEvictTraceFile(FLAGS_evict_block_cache_trace_path);
  analyzer.SetEvictTraceOutputHumanFile(FLAGS_evict_human_readable_trace_file_path);

  Status s;
  if(FLAGS_evict_block_cache_trace_path.empty()) {
    s = analyzer.Analyze();
  } else {
    s = analyzer.AnalyzeWithEvict();
  }
